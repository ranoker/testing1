...
def get_item(kitu):
    nlist = kitu.split("-")
    data = f"use_stripe_sdk=true&mandate_data[customer_acceptance][type]=online&mandate_data[customer_acceptance][online][infer_from_client]=true&return_url=https%3A%2F%2Fsmithvillelittleleague.org%2Fdonate%2F&payment_method={kitu[2]}&key=pk_live_51OctxeJWPL9ZNbRNhPoLAFUyQ9Va0NqbxvJpplzacSF6YItte3vUHvUyjh34IcklHP3JM5xnC7CODJOuXGfnas0Q00MUPtluo5&client_secret={kitu[1]}"
    ...
    res = request.post(f"https://api.stripe.com/v1/payment_intents/{kitu[0]}/confirm", data=data, headers=headers) 
    return res.json()
